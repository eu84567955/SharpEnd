...
namespace WvsData
{
    internal static class MobExport
    {
        public static void Export(string inputPath, string outputPath)
        {
            ...
            {
                using (FileStream stream = File.Create(Path.Combine(outputPath, mob.Identifier.ToString() + ".shd")))
                {
            ...
            Console.WriteLine("\t\tDone ({0}).", mobs.Count);
        }
    }
}
